...
const SignUp: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!authStore.email) newErrors.email = "이메일을 입력해주세요.";
    if (!authStore.name) newErrors.name = "이름을 입력해주세요.";
    if (!authStore.nickname) newErrors.nickname = "닉네임을 입력해주세요.";
    if (!authStore.accountBank) newErrors.accountBank = "금융기관을 선택해주세요.";
    if (!authStore.account) newErrors.account = "계좌번호를 입력해주세요.";
    if (!authStore.password) newErrors.password = "비밀번호를 입력해주세요.";
    if (authStore.password !== authStore.checkPassword) newErrors.checkPassword = "비밀번호가 일치하지 않습니다.";
    setErrors(newErrors);
  ...
  const renderInput = (
    label: string,
    name: keyof Omit<typeof authStore, "setField" | "resetForm" | "login" | "signUp">,
    type: string,
    placeholder: string,
  ) => (
    <div className="w-full sm:w-96 flex justify-between items-center mb-6">
      ...
      <input
        ...
        value={authStore.name as string}
        onChange={(e) => authStore.setField(name, e.target.value)}
      />
